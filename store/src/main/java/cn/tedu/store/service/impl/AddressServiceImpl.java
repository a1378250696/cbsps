...
public class AddressServiceImpl implements IAddressService {
	...
	public void addnew(Integer uid, String username, Address address) {
		...
		Integer add = countByUid(uid);

		// 判断数量是否达到上限
		if (add >= maxCount) {
			// 是：抛出AddressCountLimitException
			throw new AddressCountLimitException("收货地址数量达到" + maxCount + "条上限,请清除无需的收货地址!");
		}
	...
	public void setDefault(Integer aid, Integer uid, String username) {
		...
		// 调用addressMapper的findByAid()方法，根据参数aid查询收货地址数据
		Address add = findByAid(aid);

		// 判断查询结果是否为null
		if (add == null) {
			// 是：抛出AddressNotFoundException
			throw new AddressNotFoundException("收货地址为空或不存在,请添加!");
		}
	...
	public void delete(Integer aid, Integer uid, String username) {
		...
		// 调用addressMapper的deleteByAid(Integer aid)方法执行删除，并获取返回的受影响行数
		deleteByAid(aid);
		...
		// 调用addressMapper的countByUid(Integer uid)方法统计此时该用户的收货地址数据的数量
		...
		// 调用addressMapper的findLastModified(Integer uid)找出最后修改的收货地址
	...
	public Address getbyAid(Integer aid, Integer uid) {
		...
		if(!add.getUid().equals(uid)) {
			// 是：AccessDeniedException
			throw new AccessDeniedException("非法访问,数据异常!");
		}
			// 将查询结果中的4个日志属性设置为null
		add.setCreatedUser(null);
		add.setCreatedTime(null);
		add.setModifiedTime(null);
		add.setModifiedUser(null);
			// 返回查询结果
		return add;
	}
	...
	private void insert(Address address) {
		// 调用addressMapper的insert()方法，插入收货地址数据，并获取返回的受影响行数
			...
			if (rows != 1) {
				// 是：抛出InsertExcption
				throw new InsertException("插入收货地址数据时出现未知错误，请联系客服!");
			}
	}
	...
	private void updateDefaultByAid(Integer aid, String modifiedUser, Date modifiedTime) {
		// 执行设置默认收货地址
		Integer rows = addressMapper.updateDefultByAid(aid, modifiedUser, modifiedTime);
		// 判断受影响行数是否不为1
		if (rows != 1) {
			// 是：UpdateException
			throw new UpdateException("更新收货地址数据,请联系客服");
		}
	}
