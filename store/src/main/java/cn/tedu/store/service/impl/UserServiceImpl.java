...
public class UserServiceImpl implements IUserService {
	...
	public void reg(User user) {
		...
		if (result != null) {
			...
			throw new UsernameDuplicateException("该用户已被注册!");
		}

		// 是:根据用户名没有查询到数据,表示用户名没有被注册,则允许注册
		...
		// 调用userMapper的insert()方法执行注册,获取返回的搜影响行数
		Integer r = userMapper.insert(user);
		// 判断受影响的行数是否为1
		if (r != 1) {
			// 是:表示注册失败,则抛出InsertException对象
			throw new InsertException("插入用户数据未知错误,请联系客服!");
		}
	...
	public User login(String username, String password) {
		...
		// [1]将查询结果中除了uid,usename,avatar以外的属性都设置为null
		// [1]返回查询结果
	...
	public void changePassword(Integer uid, String username, String oldPassword, String newPassword) {
		...
		// 从查询结果中获取salt值
		String salt = result.getSalt();
		// 留参数oldPassword结合salt加密,得到oldMd5Password
		...
		Date now = new Date();
		// 执行修改密码，并获取受影响的行数
		Integer row = userMapper.updatePasswordByUid(uid, newMd5Password, username, now);
		// 判断受影响的行数是否不为1
		if (row != 1) {
			// 是：抛出UpdateException
			throw new UpdateException("修改数据错误");
		}
	...
	public User getInfo(Integer uid) {
		...
		// 创建新的User对象
		// 向新的User对象中封装username,phone,email,gender
		User user = new User();
		user.setUsername(result.getUsername());
